...
```bash
php artisan vendor:publish --provider="GetThingsDone\Skeleton\SkeletonServiceProvider" --tag="migrations"
php artisan migrate
```

You can publish the config file with:
```bash
php artisan vendor:publish --provider="GetThingsDone\Skeleton\SkeletonServiceProvider" --tag="config"
```
...
``` php
$skeleton = new GetThingsDone\Skeleton();
